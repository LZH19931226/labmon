 ...
 * @dochub MediaIds
 */
public final class UniqueHash implements Comparable<UniqueHash>,
     ...
     * @return the new id
     */
    public static UniqueHash get() {
        return new UniqueHash();
    }

    /**
     * Checks if a string could be an <code>MediaId</code>.
     *
    ...
    /**
     * Turn an object into an <code>MediaId</code>, if possible. Strings will be converted into <code>MediaId</code>s, if possible, and <code>MediaId</code>s
     * will be cast and returned. Passing in <code>null</code> returns <code>null</code>.
     *
     * @param o the object to convert
     * @return an <code>MediaId</code> if it can be massaged, null otherwise
     */
    public static UniqueHash massageToMediaId(Object o) {
        if (o == null) { return null; }

        if (o instanceof UniqueHash) { return (UniqueHash) o; }

        if (o instanceof String) {
            String s = o.toString();
            if (isValid(s)) { return new UniqueHash(s); }
        }

        return null;
    }

    public UniqueHash(Date time) {
        this(time, _genmachine, _nextInc.getAndIncrement());
    }

    public UniqueHash(Date time, int inc) {
        this(time, _genmachine, inc);
    }

    public UniqueHash(Date time, int machine, int inc) {
     ...
     */
    public UniqueHash(String s) {
        this(s, false);
    }

    public UniqueHash(String s, boolean babble) {

        if (!isValid(s)) { throw new IllegalArgumentException("invalid MediaId [" + s + "]"); }
     ...
     */
    public UniqueHash(int time, int machine, int inc) {
        _time = time;
        _machine = machine;
        _inc = inc;
        _new = false;
    ...
    public boolean equals(Object o) {
        ...
        UniqueHash other = massageToMediaId(o);
    ...
    public int compareTo(UniqueHash id) {
        if (id == null) { return -1; }
    ...
    static {
        ...
        try {
            // oauth2 a 2-byte machine piece based on NICs info
            int machinePiece;
            {
                try {
            ...
            final int processPiece;
            {
                ...
                ClassLoader loader   = UniqueHash.class.getClassLoader();
    ...
    public static String Id() {
        return new UniqueHash().toString();
    }

    public static void main(String[] args) {
        Set<String> ids = new HashSet<String>();
        for (int i = 0; i < 10000001; i++) {
            // String id = new MediaId().toString();
            ids.add(new UniqueHash().toString());
